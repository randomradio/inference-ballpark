(function () {
  const node = (id, stageIndex, label, kind, op, shape, detail, dependsOn = [], extra = {}) => ({
    ...
    dOut: extra.dOut,
    stack: extra.stack || 0,
    stackActive: extra.stackActive || 0,
    spine: !!extra.spine,
  });
  ...
  const glm52 = {
    ...
    nodes: [
      ...
      node("hidden_in", 0, "Hidden states", "activation", "Embedded sequence", "[{B}, {T}, 6,144]", "decoder layer 输入。", ["embedding"], { spine: true }),
      ...
      node("index_q", 1, "Indexer Q", "weight", "wq_b", "[{B}, {T}, 32, 128]", "独立 indexer query。", ["q_resid"], { costId: "indexer_q", dIn: 2048, dOut: 4096, stack: 32 }),
      ...
      node("sparse_attention", 1, "Sparse MLA", "operation", "masked QKᵀ → V", "[{B}, 64, {T}, 256]", "只读 DSA 选出的 Top-2048。", ["q_up", "kv_up", "rope", "index_share"], { costId: "attn_sparse_mla", stack: 64 }),
      ...
      node("attn_residual", 1, "Attention residual", "activation", "Residual add", "[{B}, {T}, 6,144]", "attention + layer input。", ["attn_out", "hidden_in"], { spine: true }),
      ...
      node("routed_experts", 2, "256 routed experts", "module", "Top-8 SwiGLU", "6,144 → 2,048 → 6,144", "只执行被选中的 8 个。EP=1 时在此计算。", ["router_topk", "ffn_norm"], { costId: "moe_routed", skipIfEp: true, dIn: 6144, dOut: 2048, stack: 256, stackActive: 8 }),
      node("shared_expert", 2, "Shared expert", "module", "Always-on SwiGLU", "6,144 → 2,048 → 6,144", "不受 Top-K 影响。", ["ffn_norm"], { costId: "moe_shared", dIn: 6144, dOut: 2048, stack: 1 }),
      ...
      node("expert_compute", 3, "Local expert compute", "module", "SwiGLU FFNs", "Top-8 routes", "每个 rank 执行本地专家。", ["all_to_all"], { epOnly: true, costId: "moe_routed", dIn: 6144, dOut: 2048, stack: 256, stackActive: 8 }),
      ...
      node("block_residual", 3, "Block residual", "activation", "FFN + residual", "[{B}, {T}, 6,144]", "进入下一层。", ["gather", "moe_combine", "shared_expert", "attn_residual"], { spine: true }),

      node("final_norm", 4, "Final RMSNorm", "operation", "Normalize last token", "[{B}, 6,144]", "只用最后一个位置。", ["block_residual"], { costId: "rms", once: true, dIn: 6144, spine: true }),
      ...
      node("logits", 4, "Logits", "activation", "Last-token scores", "[{B}, 154,880]", "采样一个 token。", ["lm_head"], { once: true, spine: true }),
    ],
  };
  ...
  const kimiK3 = {
    ...
    nodes: [
      ...
      node("hidden_in", 0, "Hidden states", "activation", "Embedded sequence", "[{B}, {T}, 7,168]", "decoder 输入。视觉路径不计入文本吞吐。", ["embedding"], { spine: true }),

      node("attn_norm", 1, "RMSNorm", "operation", "Pre-attention norm", "[{B}, {T}, 7,168]", "attention 前归一化。", ["hidden_in"], { costId: "rms", dIn: 7168 }),
      node("kda_qkv", 1, "KDA Q / K / V", "weight", "Three projections", "[{B}, {T}, 96, 128]", "96 × 128 维 QKV。", ["attn_norm"], { layerBranch: "kda", costId: "kda_qkv", dIn: 7168, dOut: 36864, stack: 96 }),
      ...
      node("kda_gate", 1, "KDA gates", "weight", "full-rank gate + beta", "[{B}, {T}, 96, 128]", "控制 delta update 与遗忘。", ["attn_norm", "short_conv"], { layerBranch: "kda", costId: "kda_gate", dIn: 7168, dOut: 12288, stack: 96 }),
      ...
      node("gated_mla", 1, "Gated MLA", "operation", "RoPE attention × gate", "[{B}, {T}, 7,168]", "完整 MLA 再乘 output gate。", ["mla_q", "mla_cache"], { layerBranch: "mla", costId: "attn_gated_mla", stack: 96 }),
      node("attention_output", 1, "Attention output", "activation", "Selected branch", "[{B}, {T}, 7,168]", "当前层类型的 KDA 或 MLA 输出。", ["kda_output", "gated_mla"], { spine: true }),
      ...
      node("routed_experts", 2, "896 latent experts", "module", "16 active SiTU", "3,584 → 3,072 → 3,584", "在 latent width 内计算。", ["top16", "latent_down"], { costId: "moe_routed", skipIfEp: true, dIn: 3584, dOut: 3072, stack: 896, stackActive: 16 }),
      node("shared_experts", 2, "2 shared experts", "module", "Always-on latent MLPs", "3,584 → 2×3,072 → 3,584", "两个 shared experts 始终执行。", ["latent_down"], { costId: "moe_shared", dIn: 3584, dOut: 3072, stack: 2 }),
      ...
      node("expert_compute", 3, "Local latent experts", "module", "16 active SiTU", "3,584 → 3,072 → 3,584", "本地专家计算。", ["all_to_all"], { epOnly: true, costId: "moe_routed", dIn: 3584, dOut: 3072, stack: 896, stackActive: 16 }),
      ...
      node("block_output", 3, "Block output", "activation", "AttnRes + MoE", "[{B}, {T}, 7,168]", "进入下一 decoder layer。", ["latent_up", "attnres_bank"], { spine: true }),

      node("final_norm", 4, "Final RMSNorm", "operation", "Normalize last token", "[{B}, 7,168]", "只用最后位置。", ["block_output"], { costId: "rms", once: true, dIn: 7168, spine: true }),
      ...
      node("logits", 4, "Logits", "activation", "Last-token scores", "[{B}, 163,840]", "采样一个 token。", ["lm_head"], { once: true, spine: true }),
    ],
  };
  ...
  const kimiK25 = {
    ...
    nodes: [
      ...
      node("hidden_in", 0, "Hidden states", "activation", "Embedded sequence", "[{B}, {T}, 7,168]", "decoder 输入。", ["embedding"], { spine: true }),
      ...
      node("mla_attn", 1, "MLA attention", "operation", "QKᵀ → softmax → V", "[{B}, 64, {T}, {S}]", "组合 NoPE 与 RoPE 子空间。", ["q_up", "kv_up"], { costId: "attn_mla_full", stack: 64 }),
      node("o_proj", 1, "Output Wᴼ", "weight", "8,192 → 7,168", "[8,192 × 7,168]", "多头 context 投影回 H。", ["mla_attn"], { costId: "gemm", dIn: 8192, dOut: 7168 }),
      node("attn_residual", 1, "Attention residual", "activation", "Residual add", "[{B}, {T}, 7,168]", "attention + input。", ["o_proj", "hidden_in"], { spine: true }),
      ...
      node("routed_experts", 2, "384 routed experts", "module", "8 active SwiGLU", "7,168 → 2,048 → 7,168", "EP=1 时在此计算。", ["topk", "ffn_norm"], { costId: "moe_routed", skipIfEp: true, dIn: 7168, dOut: 2048, stack: 384, stackActive: 8 }),
      node("shared_expert", 2, "Shared expert", "module", "Always-on SwiGLU", "7,168 → 2,048 → 7,168", "始终处理 token。", ["ffn_norm"], { costId: "moe_shared", dIn: 7168, dOut: 2048, stack: 1 }),
      ...
      node("expert_compute", 3, "Local experts", "module", "8 active SwiGLU", "7,168 → 2,048 → 7,168", "本地专家。", ["all_to_all"], { epOnly: true, costId: "moe_routed", dIn: 7168, dOut: 2048, stack: 384, stackActive: 8 }),
      node("gather", 3, "Inverse gather", "communication", "restore order", "[{B}, {T}, 8, 7,168]", "写回 token/expert slot。", ["expert_compute"], { epOnly: true }),
      node("block_residual", 3, "Block residual", "activation", "FFN + residual", "[{B}, {T}, 7,168]", "进入下一层。", ["gather", "moe_combine", "shared_expert", "attn_residual"], { spine: true }),

      node("final_norm", 4, "Final RMSNorm", "operation", "Normalize last token", "[{B}, 7,168]", "只用最后位置。", ["block_residual"], { costId: "rms", once: true, dIn: 7168, spine: true }),
  ...
  const minimaxM3 = {
    ...
    nodes: [
      ...
      node("hidden_in", 0, "Hidden states", "activation", "Embedded sequence", "[{B}, {T}, 6,144]", "decoder 输入。", ["embedding"], { spine: true }),

      node("attn_norm", 1, "Gemma RMSNorm", "operation", "Pre-attention norm", "[{B}, {T}, 6,144]", "Gemma-style RMSNorm。", ["hidden_in"], { costId: "rms", dIn: 6144 }),
      node("q_proj", 1, "Q projection", "weight", "64 heads × 128", "[{B}, {T}, 64, 128]", "主 attention queries。", ["attn_norm"], { costId: "gemm", dIn: 6144, dOut: 8192, stack: 64 }),
      node("kv_proj", 1, "K / V projections", "weight", "4 KV heads × 128", "[{B}, {T}, 4, 128]", "GQA：每 16 个 Q 共享 1 组 KV。Prefill 写入 paged cache。", ["attn_norm"], { cacheRole: "write", costId: "gemm", dIn: 6144, dOut: 1024, stack: 4 }),
      node("kv_cache", 1, "Paged KV cache", "cache", "full sequence storage", "[{B}, {S}, 4, 256]", "显存按全量 S 计；Decode 计算只读选中 blocks。", ["kv_proj"], { cacheRole: "readAppend", costId: "kv_cache_gqa", stack: 4 }),
      node("indexer", 1, "Lightning indexer", "module", "4 heads × 128", "block scores", "对 KV blocks 打分。", ["attn_norm"], { costId: "m3_indexer", stack: 4 }),
      node("index_topk", 1, "Top-16 KV blocks", "operation", "select + local block", "16 × 128 + local", "稀疏单位是 block 不是单个 token。", ["indexer"]),
      node("sparse_gqa", 1, "Sparse GQA", "operation", "attend selected blocks", "[{B}, 64, {T}, 128]", "读取 indexer 选择的 paged KV。", ["q_proj", "kv_cache", "index_topk"], { costId: "attn_sparse_gqa", stack: 64 }),
      node("attn_out", 1, "Output projection", "weight", "64×128 → 6,144", "[8,192 × 6,144]", "投影回 H。", ["sparse_gqa"], { costId: "gemm", dIn: 8192, dOut: 6144 }),
      node("attn_residual", 1, "Attention residual", "activation", "Residual add", "[{B}, {T}, 6,144]", "attention + input。", ["attn_out", "hidden_in"], { spine: true }),
      ...
      node("routed_experts", 2, "128 routed experts", "module", "4 active SwiGLU-OAI", "6,144 → 3,072 → 6,144", "EP=1 时在此计算。", ["top4", "ffn_norm"], { costId: "moe_routed", skipIfEp: true, dIn: 6144, dOut: 3072, stack: 128, stackActive: 4 }),
      node("shared_expert", 2, "Shared expert", "module", "Always-on SwiGLU-OAI", "6,144 → 3,072 → 6,144", "始终激活。", ["ffn_norm"], { costId: "moe_shared", dIn: 6144, dOut: 3072, stack: 1 }),
      ...
      node("expert_compute", 3, "Local experts", "module", "4 active SwiGLU", "6,144 → 3,072 → 6,144", "本地专家。", ["all_to_all"], { epOnly: true, costId: "moe_routed", dIn: 6144, dOut: 3072, stack: 128, stackActive: 4 }),
      node("inverse_gather", 3, "Inverse gather", "communication", "restore order", "[{B}, {T}, 4, 6,144]", "恢复 token 顺序。", ["expert_compute"], { epOnly: true }),
      node("block_residual", 3, "Block residual", "activation", "FFN + residual", "[{B}, {T}, 6,144]", "进入下一层。", ["inverse_gather", "moe_combine", "shared_expert", "attn_residual"], { spine: true }),

      node("final_norm", 4, "Final Gemma RMSNorm", "operation", "Normalize last token", "[{B}, 6,144]", "只用最后位置。", ["block_residual"], { costId: "rms", once: true, dIn: 6144, spine: true }),
      node("lm_head", 4, "LM head", "weight", "6,144 → 200,064", "[6,144 × 200,064]", "投影到词表。", ["final_norm"], { costId: "gemm", once: true, dIn: 6144, dOut: 200064 }),
      node("logits", 4, "Logits", "activation", "Last-token scores", "[{B}, 200,064]", "采样一个 token。", ["lm_head"], { once: true, spine: true }),
    ],
  };
